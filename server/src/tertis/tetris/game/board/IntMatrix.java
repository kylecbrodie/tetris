...
public final class IntMatrix implements Cloneable {
	...
	/**
	 * Constructs an IntMatrix with all elements unoccupied.
	 */
	 ...
	 */
	public boolean canContain(IntMatrix other, Position pos) {
		return canPartlyContain(other, pos, 0);
	}

	/**
	 * Return true if part of the other IntMatrix can be placed in this one.
	 */
	public boolean canPartlyContain(IntMatrix other, Position pos, int begin) {
		...
		for (int i = begin; i < other.getHeight(); i++) {
			for (int j = 0; j < other.getWidth(); j++) {
				if (other.get(i, j) > 0 && this.get(i + pos.getRow() - begin, j + pos.getColumn()) > 0) {
					return false;
				}
			}
		}
